...
void output_a_line_new(FILE *fout, char *line, SYLLABLE_CODE *sylcode, LENDICT *ld, LEXICON *lex, WMOSYM_LIST *wait_for_free);
void load_text_list(FILE *flst, TEXT_LIST *tl);
void run(FILE *ftls, FILE *fwls, char *data_kind, FILE *fdic, int speaker_split_flag, char *output_dir);
void gen_dir(char *output_dir, char *data_kind);
void reset_dir_data(char *dir);
void load_data(FILE *ftls, FILE *fwls, DATA_SET *ds);
...
void output_data(char *output_dir, UTT2SPK *train, UTT2SPK *dev, UTT2SPK *test, char **diceing_dir, TEXT_AND_WAV *useful, LEXICON *lex, LENDICT *ld, WMOSYM_LIST *wait_for_free, SYLLABLE_CODE *sylcode);
void output_utt2spk(UTT2SPK *u2s, char *output_path);
void build_lexicon(FILE *fdic, LEXICON *lex, LENDICT *ld, char *output_path);
...
void add_wmosym_list(WMOSYM_LIST *obl, WMOSYM **ob);
void destroy_wmosym_list(WMOSYM_LIST *obl);
void extern_wmosym_list(WMOSYM_LIST *obl);
void ini_wmosym_list(WMOSYM_LIST *obl);
void destroy_wmosym(WMOSYM **top);
void normalize_word(char *raw, char **modify);
int bin_lexicon_search(char *word, LEXICON *lex);
void get_all_sylcode(char *line, SYLLABLE_CODE *sylcode, LENDICT *ld, LEXICON *lex, WMOSYM_LIST *wait_for_free);
int bin_lexicon_search_len_match(char *word, LEXICON *lex);
int isEnglish(char *word);
void ini_sylcode(SYLLABLE_CODE *sylcode);
void maximux_length_first(char *line, SYLLABLE_CODE *sylcode, LENDICT *ld, WMOSYM_LIST *wait_for_free);
void recursive_viterbi(LENDICT *ld, WMOSYM **pre_state, WMOSYM **most_likely, char *raw, int acc_idx, WMOSYM_LIST *wait_for_free, WORD_LEN_RECORD *wlr, int maximum_length);
void get_all_phonemes(char *output_dir, PHONEME_SET *ps, LEXICON *lex);
void training_data_output(char *output_dir, UTT2SPK *train, UTT2SPK *dev, UTT2SPK *test, char **diceing_dir, TEXT_AND_WAV *useful, LEXICON *lex, LENDICT *ld, WMOSYM_LIST *wait_for_free, SYLLABLE_CODE *sylcode);
...
int main(int argc, char **argv)
{
	...
	if (argc != 7)
	{
		printf("Usage: excute.exe text_list wav_list kind_of_data_set(train/test/dev/all) dictionary split_speaker(0/1) s5_dir\n");
	...
	if (!(fdic = fopen(argv[4], "r")))
	{
		printf("can not open file: %s\n", argv[4]);
		exit(1);
	}
	srand(time(NULL));
	run(ftls, fwls, argv[3], fdic, atoi(argv[5]), argv[6]);
...
void run(FILE *ftls, FILE *fwls, char *data_kind, FILE *fdic, int speaker_split_flag, char *output_dir)
{
	...
	ini_wmosym_list(&wait_for_free);
	ini_sylcode(&sylcode);
	gen_dir(checked_output_dir, data_kind);
	load_data(ftls, fwls, &ds);
	build_lexicon(fdic, &lex, &ld, checked_output_dir);
	...
	output_data(checked_output_dir, &ds.train, &ds.dev, &ds.test, ds.diceing_dir, &ds.useful, &lex, &ld, &wait_for_free, &sylcode);
...
void output_data(char *output_dir, UTT2SPK *train, UTT2SPK *dev, UTT2SPK *test, char **diceing_dir, TEXT_AND_WAV *useful, LEXICON *lex, LENDICT *ld, WMOSYM_LIST *wait_for_free, SYLLABLE_CODE *sylcode)
{
	training_data_output(output_dir, train, dev, test, diceing_dir, useful, lex, ld, wait_for_free, sylcode);
}
void output_a_line_new(FILE *fout, char *line, SYLLABLE_CODE *sylcode, LENDICT *ld, LEXICON *lex, WMOSYM_LIST *wait_for_free)
{
	int i, idx, Eflag;
	char word[7];
	char *nor_line = (char *)malloc((strlen(line) + 1) * sizeof(char));
	char *sub_line = (char *)malloc((strlen(line) + 1) * sizeof(char));

	normalize_a_line(line, nor_line);
	//printf("%s\n", nor_line);
	for (idx = 0, Eflag = 0; Eflag == 0;)
	{
		Eflag = get_sub_line(nor_line, sub_line, &idx);
		
		if (sub_line[0] >= 65 && sub_line[0] <= 90)
		{
			fprintf(fout, "%s ", sub_line);
		}
		else if (sub_line[0] != ' ')
		{
			get_all_sylcode(sub_line, sylcode, ld, lex, wait_for_free);
			for (i = 0; i < sylcode->num_lex; i++)
			{
				fprintf(fout, "%s ", sylcode->lex[i]);
			}
		}
	}
	...
	for (sub[0] = '\0'; read_a_utf8_word(line, word, idx) == 0;)
	{
		...
		else if (isEnglish(word) == 1)
		{
			...
			for (; read_a_utf8_word(line, word, idx) == 0;)
			{
				...
				else if (isapostrophe(word) == 1 || isDot(word) == 1)
				{
					read_a_utf8_word(line, next, idx);
					if (isEnglish(next) == 1)
					{
						strcat(sub, word);
						Retreat_a_utf8_word(line, idx);
...
void training_data_output(char *output_dir, UTT2SPK *train, UTT2SPK *dev, UTT2SPK *test, char **diceing_dir, TEXT_AND_WAV *useful, LEXICON *lex, LENDICT *ld, WMOSYM_LIST *wait_for_free, SYLLABLE_CODE *sylcode)
{
	...
	for (i = 0; i < useful->tl.num_of_text; i++)
	{
		strcpy(path, output_dir);
		strcat(path, diceing_dir[i]);
		strcat(path, "text");
		if (!(fout = fopen(path, "a")))
		{
			printf("can not open file: %s\n", path);
			exit(1);
		}
		...
		{
			//fprintf(fout, "%s <SILENCE> ", useful->tl.text_name[i]);
			fprintf(fout, "%s ", useful->tl.text_name[i]);
			for (j = 0; j < useful->tl.txtlst[i].num_of_sentence; j++)
			{
				//get_all_sylcode(useful->tl.txtlst[i].sentence[j], sylcode, ld, lex, wait_for_free);
				//for (k = 0; k < sylcode->num_lex; k++)
				//{
				//	fprintf(fout, "%s ", sylcode->lex[k]);
				//}
				//output_a_line(fout, useful->tl.txtlst[i].sentence[j]);
				output_a_line_new(fout, useful->tl.txtlst[i].sentence[j], sylcode, ld, lex, wait_for_free);
			}
		...
		{
			fprintf(fout, "%s %s\n", useful->wl.wav_name[i], useful->wl.wav_full_name[i]);
			fclose(fout);
		}
	}
	if (train->utt_num > 0)
	{
		strcpy(path, output_dir);
		strcat(path, "data/train/utt2spk");
		output_utt2spk(train, path);
	}
	if (dev->utt_num > 0)
	{
		strcpy(path, output_dir);
		strcat(path, "data/dev/utt2spk");
		output_utt2spk(dev, path);
	}
	if (test->utt_num > 0)
	{
		strcpy(path, output_dir);
		strcat(path, "data/test/utt2spk");
		output_utt2spk(test, path);
	}
}
void output_utt2spk(UTT2SPK *u2s, char *output_path)
{
...
void ini_wmosym_list(WMOSYM_LIST *obl)
{
	obl->size = 0;
	obl->capacity = 1024;
	obl->list = (WMOSYM **)malloc(obl->capacity * sizeof(WMOSYM *));
}
void extern_wmosym_list(WMOSYM_LIST *obl)
{
	obl->capacity += 1024;
	obl->list = (WMOSYM **)realloc(obl->list, obl->capacity * sizeof(WMOSYM *));
}
void destroy_wmosym_list(WMOSYM_LIST *obl)
{
	free(obl->list);
...
void add_wmosym_list(WMOSYM_LIST *obl, WMOSYM **ob)
{
	obl->list[obl->size++] = *ob;
	if (obl->size == obl->capacity)
	{
		extern_wmosym_list(obl);
	}
}
void get_all_sylcode(char *line, SYLLABLE_CODE *sylcode, LENDICT *ld, LEXICON *lex, WMOSYM_LIST *wait_for_free)
{
	char *raw = (char *)malloc((strlen(line) + 1) * sizeof(char));
	char tok[7];
	int i, idx, temp;
	int ret, flag, raw_utf8len, utf8_idx;

	//get_raw_line(line, raw);
	strcpy(raw, line);
	raw_utf8len = utf8_word_length(raw);
	//printf("%s\n", raw);

	if (raw_utf8len > sylcode->capacity)
	{
		sylcode->code = (int *)realloc(sylcode->code, raw_utf8len * sizeof(int));
		sylcode->isword = (int *)realloc(sylcode->code, raw_utf8len * sizeof(int));
		sylcode->isexist = (int *)realloc(sylcode->code, raw_utf8len * sizeof(int));
		sylcode->start_fill_idx = (int *)realloc(sylcode->code, raw_utf8len * sizeof(int));
		sylcode->wmregion = (int **)realloc(sylcode->wmregion, raw_utf8len * sizeof(int *));
		sylcode->word = (char **)realloc(sylcode->word, raw_utf8len * sizeof(char *));
		sylcode->lex = (char **)realloc(sylcode->word, raw_utf8len * sizeof(char *));
		for (i = sylcode->capacity; i < raw_utf8len; i++)
		{
			sylcode->wmregion[i] = (int *)malloc(2 * sizeof(int));
			sylcode->word[i] = (char *)malloc(7 * sizeof(char));
			sylcode->lex[i] = (char *)malloc(sizeof(char));
		}
		sylcode->capacity = raw_utf8len;
	}

	for (idx = 0, sylcode->size = utf8_idx = 0, flag = 0, sylcode->region_num = 0;; utf8_idx++)
	{
		if (read_a_utf8_word(raw, tok, &idx) == 1)
		{
			if (flag == 1)
			{
				sylcode->wmregion[sylcode->region_num++][1] = idx;
				flag = 0;
			}
			break;
		}
		strcpy(sylcode->word[sylcode->size], tok);

		ret = bin_lexicon_search_len_match(tok, &(ld->lexs[strlen(tok) - 1]));
		if (ret == -1)
		{
			sylcode->isexist[sylcode->size] = 0;
			if (isCJK_words(tok) == 1)
			{
				sylcode->isword[sylcode->size] = 1;
				sylcode->code[sylcode->size] = 1008;
				printf("warring: dict not include %s\n", tok);
			}
			else
			{
				sylcode->isword[sylcode->size] = 0;
				sylcode->code[sylcode->size] = 6100;
			}
			
		}
		if ((isCJK_words(tok) == 1 || isnumber(tok) == 1) && ret != -1)
		{
			sylcode->isword[sylcode->size] = 1;
			if (flag == 0)
			{
				temp = idx;
				Retreat_a_utf8_word(raw, &temp);
				sylcode->start_fill_idx[sylcode->region_num] = utf8_idx;
				sylcode->wmregion[sylcode->region_num][0] = temp;
				flag = 1;
			}
		}
		else
		{
			...
			{
				sylcode->isexist[sylcode->size] = 0;
				sylcode->code[sylcode->size] = atoi(ld->lexs[strlen(tok) - 1].sw[ret].syl_code_seq);
			}
			else
			{
				//printf("error: ret should not be -1 here.\nword = %s\n", tok);
				//exit(1);
			}
			if (flag == 1)
			{
				temp = idx;
				Retreat_a_utf8_word(raw, &temp);
				sylcode->wmregion[sylcode->region_num++][1] = temp;
				flag = 0;
			}
		}
		sylcode->size++;
	}
	maximux_length_first(raw, sylcode, ld, wait_for_free);

	/*printf("%s %d\n", raw, utf8_word_length(raw));
    for (i = 0; i < sylcode->region_num; i++)
    {
        printf("(%d,%d) ", sylcode->wmregion[i][0], sylcode->wmregion[i][1]);
    }
    printf("\n");*/
	/*for (i = idx = 0; read_a_utf8_word(raw, tok, &idx) == 0; i++)
    {
        printf("%s\t%d\t%d\t%d\n", tok, sylcode->isword[i], sylcode->code[i], sylcode->isexist[i]);
    }*/
	free(raw);
}
int bin_lexicon_search_len_match(char *word, LEXICON *lex)
...
void maximux_length_first(char *line, SYLLABLE_CODE *sylcode, LENDICT *ld, WMOSYM_LIST *wait_for_free)
{
	int i, j, k, syl_idx, search_len;
	WORD_LEN_RECORD wlr;
	char *temp = (char *)malloc((strlen(line) + 1) * sizeof(char));
	WMOSYM *ini_osym, *most_likely;
	WMOSYM *wmtemp;
	char Asyllable[5] = {'\0'};
	int ret, num_lex;

	wlr.size = strlen(line);
	wlr.cur_len = (int *)malloc(wlr.size * sizeof(int));
	wlr.max_len = (int *)malloc(wlr.size * sizeof(int));
	wlr.link_num = (int *)malloc(wlr.size * sizeof(int));
	wlr.acc_prob = (double *)malloc(wlr.size * sizeof(double));

	for (i = 0, sylcode->num_lex = 0; i < sylcode->region_num; i++)
	{
		//printf("%d %d\t", sylcode->wmregion[i][0], sylcode->wmregion[i][1]);
		temp[sylcode->wmregion[i][1] - sylcode->wmregion[i][0]] = '\0';
		strncpy(temp, &(line[sylcode->wmregion[i][0]]), sylcode->wmregion[i][1] - sylcode->wmregion[i][0]);
		//printf("%s\t", temp);

		ini_osym = NULL;
		most_likely = NULL;
		wait_for_free->size = 0;
		for (j = 0; j < wlr.size; j++)
		{
			wlr.cur_len[j] = 0;
			wlr.max_len[j] = 0;
			wlr.link_num[j] = 0;
		...
		recursive_viterbi(ld, &ini_osym, &most_likely, temp, 1, wait_for_free, &wlr, 0);

		for (wmtemp = most_likely, num_lex = 0; wmtemp != NULL; wmtemp = wmtemp->prev)
		{
			num_lex++;
		}
		for (wmtemp = most_likely, syl_idx = utf8_word_length(temp) - 1, k = 1; wmtemp != NULL; wmtemp = wmtemp->prev, k++)
		{
			sylcode->lex[sylcode->num_lex + num_lex - k] = (char *)realloc(sylcode->lex[sylcode->num_lex + num_lex - k], (strlen(wmtemp->word) + 1) * sizeof(char));
			strcpy(sylcode->lex[sylcode->num_lex + num_lex - k], wmtemp->word);
			//printf("%s ", wmtemp->word);
			ret = bin_lexicon_search_len_match(wmtemp->word, &(ld->lexs[strlen(wmtemp->word) - 1]));
			search_len = utf8_word_length(wmtemp->word);
			for (j = 0; j < search_len; j++, syl_idx--)
			{
				strncpy(Asyllable, &(ld->lexs[strlen(wmtemp->word) - 1].sw[ret].syl_code_seq[(search_len - j - 1) * 4]), 4);
				sylcode->code[sylcode->start_fill_idx[i] + syl_idx] = atoi(Asyllable);
				sylcode->isexist[sylcode->start_fill_idx[i] + syl_idx] = (search_len > 1) ? 1 : 0;
				//printf("(%d %d)", sylcode->start_fill_idx[i] + syl_idx, sylcode->code[sylcode->start_fill_idx[i] + syl_idx]);
			}

			//printf("%s %s %d\n", wmtemp->word, ld->lexs[strlen(wmtemp->word) - 1].sw[ret].syl_code_seq, search_len);
		}
		sylcode->num_lex += num_lex;
		//printf("\n");
		for (j = 0; j < wait_for_free->size; j++)
		{
			destroy_wmosym(&(wait_for_free->list[j]));
		}
		destroy_wmosym(&most_likely);
	}
...
{
	int i, j, ret;
	int word_len = strlen(raw), max_len;
	int max_now, own_len;
	int idx;
	char tok[7];
	int now_link_num, max_link_num;
	double now_prob, max_prob;
	WMOSYM *osym;
	WMOSYM *wmtemp;
	//if(word_len-1>ld->maximum_len){max_len=ld->maximum_len;}
	max_len = (word_len > ld->maximum_len) ? ld->maximum_len : word_len;
	//printf("max_len=%d word_len=%d acc_idx=%d %s\n", max_len, word_len, acc_idx, raw);
	if (word_len <= 0)
	{
		for (now_link_num = 0, wmtemp = (*pre_state); wmtemp != NULL; now_link_num++, wmtemp = wmtemp->prev)
		{
			continue;
		}
		for (max_link_num = 0, wmtemp = (*most_likely); wmtemp != NULL; max_link_num++, wmtemp = wmtemp->prev)
		{
			continue;
		}
		if (*most_likely == NULL)
		{
			(*most_likely) = (*pre_state);
			wlr->max_len[acc_idx - 1] = maximum_length;
		}
		else
		{
			if (wlr->max_len[acc_idx - 1] < maximum_length)
			{
				add_wmosym_list(wait_for_free, most_likely);
				(*most_likely) = (*pre_state);
				wlr->max_len[acc_idx - 1] = maximum_length;
			}
			else if (wlr->max_len[acc_idx - 1] == maximum_length && now_link_num < max_link_num)
			{
				add_wmosym_list(wait_for_free, most_likely);
				(*most_likely) = (*pre_state);
			}
			else if (wlr->max_len[acc_idx - 1] == maximum_length && now_link_num == max_link_num && (*most_likely)->prob < (*pre_state)->prob)
			{
				add_wmosym_list(wait_for_free, most_likely);
				(*most_likely) = (*pre_state);
			}
			...
			{
				add_wmosym_list(wait_for_free, pre_state);
			}
		}
		return;
	}
	for (i = idx = 0; idx < max_len; i++)
	{
		read_a_utf8_word(raw, tok, &idx);
		ret = bin_lexicon_search_len_match(raw, &(ld->lexs[idx - 1]));
		//printf("%s %d %d\n", raw, i + 1, idx);
		if (ret != -1)
		{
			//printf("%s\t", ld->lexs[idx - 1].sw[ret].word);
			osym = (WMOSYM *)malloc(sizeof(WMOSYM));
			osym->word = (char *)malloc((strlen(ld->lexs[idx - 1].sw[ret].word) + 1) * sizeof(char));
			strcpy(osym->word, ld->lexs[idx - 1].sw[ret].word);
			osym->idx = acc_idx;
			osym->num_next = 0;
			osym->prev = *pre_state;
			own_len = strlen(osym->word);
			max_now = (own_len > maximum_length) ? own_len : maximum_length;
			if (osym->prev == NULL)
			{
				osym->prob = ld->lexs[idx - 1].sw[ret].prob_outcome;
			}
			else
			{
				osym->prev->num_next++;
				osym->prob = osym->prev->prob + ld->lexs[idx - 1].sw[ret].prob_outcome;
			}
			for (now_link_num = 0, wmtemp = osym; wmtemp != NULL; now_link_num++, wmtemp = wmtemp->prev)
			{
				continue;
			}
			//printf("o1 %d %d\t", max_now, wlr->max_len[acc_idx + i - 1]);
			//printf("o2 %d %d\t", wlr->link_num[acc_idx + i - 1], now_link_num);
			//printf("o3 %f %f\n", wlr->acc_prob[acc_idx + i - 1], osym->prob);
			if (max_now > wlr->max_len[acc_idx + i - 1])
			{
				wlr->max_len[acc_idx + i - 1] = max_now;
				wlr->cur_len[acc_idx + i - 1] = own_len;
				wlr->link_num[acc_idx + i - 1] = now_link_num;
				wlr->acc_prob[acc_idx + i - 1] = osym->prob;
				recursive_viterbi(ld, &osym, most_likely, &(raw[idx]), acc_idx + i + 1, wait_for_free, wlr, max_now);
			}
			else if (max_now == wlr->max_len[acc_idx + i - 1] && wlr->link_num[acc_idx + i - 1] > now_link_num)
			{
				wlr->max_len[acc_idx + i - 1] = max_now;
				wlr->cur_len[acc_idx + i - 1] = own_len;
				wlr->link_num[acc_idx + i - 1] = now_link_num;
			...
			else if (wlr->link_num[acc_idx + i - 1] == now_link_num && max_now == wlr->max_len[acc_idx + i - 1] && wlr->acc_prob[acc_idx + i - 1] < osym->prob)
			{
				wlr->cur_len[acc_idx + i - 1] = own_len;
				wlr->link_num[acc_idx + i - 1] = now_link_num;
				wlr->acc_prob[acc_idx + i - 1] = osym->prob;
			...
			{
				add_wmosym_list(wait_for_free, &osym);
			}
		}
		else
		{
